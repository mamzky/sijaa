...
const DeliveryDetail = () => {
  ...
  const proceedOrder = (status) => {
    ...
    updateDoc(oldData, newData).then((val) => {
      setLoading(false);
      addLog(
        "ORDER DALAM PENGANTARAN",
        `${localStorage.getItem(Constant.USERNAME)} mengantar order`
      );
      navigate(-1);
      setLoading(false);
    });
  ...
  const handleCloseDelivery = (data) => {
    ...
    updateDoc(oldData, newData).then((val) => {
      ...
      addLog(
        "ORDER SELESAI",
        `${localStorage.getItem(Constant.USERNAME)} menyelesaikan order`
      );
      navigate(-1);
    });
  };
  ...
  const getCustomer = async () => {
    ...
    const sortedData = data.docs.map((doc) => ({
      id: doc.id,
      label: `${doc.data().name}(${doc.data().contact_person})`,
      value: doc.data(),
    }));
    setCustomerList(
  ...
  const addSoldItem = (item) => {
    setSoldItem((e) => [
      ...e,
      {
        ...
        qty: 0,
        id: item?.id,
        unique_id: Math.floor(Math.random() * 9999),
      },
    ]);
  };
  ...
  const validateResult = () => {
    const validateQty = orderData?.order_list?.map((order_list, idx) => {
      const totalQty = soldItem
        ?.filter((item) => item.id === order_list?.id)
    ...
    try {
      await Promise.all(
        arrayPayload.map(async (body) => {
          ...
          await addLog(
            localStorage.getItem(Constant.USERNAME),
            `create order ${body.orderNumber}`
          );
          console.log(
            `✅ Order ${body.orderNumber} berhasil disubmit dengan ID: ${docRef.id}`
          );
        })
      );
      setTimeout(() => {
        const sumSoldQty = newOrderList?.map((item) => ({
          ...
          name: item?.name,
          sold:
            item.sold?.reduce((total, sale) => total + (sale.qty || 0), 0) || 0,
        }));
        const itemRemaining = calculateRemainingQty(
  ...
  const saveOrderSales = () => {
    validateResult();
    const newOrderList = orderData?.order_list?.map((item) => ({
      ...item,
      sold: soldItem?.filter((e) => e.id === item?.id),
    }));
    ...
    if (hasNullCustomer) {
      alert(
        "Data penjualan belum valid, terdapat transaksi dengan jumlah (qty) yang belum terisi atau pelanggan (customer) belum ditentukan."
      );
    } else {
  ...
  return (
    <div>
      ...
      <div className="container-fluid py-4">
        <div style={{ display: "flex", flexDirection: "row" }}>
          <div className="col-lg-6 col-md-3 mb-md-0 mb-4">
            ...
            <h4 className="mt-6">{`Tujuan: ${
              orderData?.customer?.name ?? ""
            } - ${orderData?.customer?.contact_person ?? ""}(${
          ...
          <div className="card-body px-0 pb-2">
            ...
            >
              <Button
                type="button"
                variant={
                  orderData?.status === "READY TO DELIVERY"
                    ? "primary"
                    : "success"
                }
                style={{ width: "20%" }}
                onClick={() => {
                  if (orderData?.status === "READY TO DELIVERY") {
                    ...
                    orderData?.type.toUpperCase() === "SALES CANVASER"
                  ) {
      ...
      <Modal show={modalResult} size="lg" centered>
        <Modal.Header>
          <Modal.Title>Status Pesanan</Modal.Title>
          ...
          <div style={{ display: "flex", flex: 1, flexDirection: "column" }}>
            <div className="table-responsive" style={{ overflowY: "auto" }}>
              <table className="table align-items-center mb-0">
                ...
                <tbody
                  style={{
                    visibility: orderDataHolder?.order_list?.length > 0,
                  }}
                >
                  {orderDataHolder?.order_list?.map((item, index) => {
                    return (
                      <tr
                        className={`border border-neutral-100 table ${
                          Boolean(overQty && overQty[index])
                            ? "table-danger"
                            : ""
                        }`}
                      >
                                      ...
                                      <Form.Select
                                        ...
                                        onChange={(inputCustomer) => {
                                          const customer = customerList.find(
                                            (e) =>
                                              e.label ===
                                              inputCustomer?.target.value
                                          )?.value;
                                          const isExist = soldItem?.findIndex(
                                            (e) =>
                                              e?.id === item?.id &&
                                          ...
                                          if (isExist === -1) {
                                            ...
                                            alert(
                                              "Customer sudah terpilih dengan item yang sama"
                                            );
                                            setSoldItem((e) =>
                                              e.filter(
                                                (item) =>
        ...
        <Modal.Footer>
          <Button variant="danger" onClick={() => setModalResult(false)}>
            Batal
          </Button>
          <Button
            variant="success"
            disabled={overQty?.some((e) => e)}
            onClick={saveOrderSales}
          >
            Selesai
          </Button>
        </Modal.Footer>
      ...
      >
        ...
        <Modal.Footer>
          <Button
            variant="danger"
            onClick={() => setShowModalCloseSales(false)}
          >
            Batal
          </Button>
          <Button
      ...
      >
        <Modal.Header>
          <Modal.Title>Selesaikan Pesanan</Modal.Title>
            ...
            <table className="table align-items-center mb-0">
              ...
              <tbody style={{ visibility: orderData?.order_list?.length > 0 }}>
                {orderData?.order_list?.map((item, index) => {
                  return (
                    <tr
                      className="border border-neutral-100 h-4"
                      style={{ background: "#6FC276" }}
                    >
                      <td>
                        <div className="ps-3 py-1">
                          <div className="d-flex flex-column justify-content-center">
        ...
        <Modal.Footer>
          <Button variant="danger" onClick={() => setModalDelivered(false)}>
            Batal
          </Button>
          <Button
            variant="success"
            onClick={() => handleCloseDelivery(orderData)}
          >
            Selesai
          </Button>
        </Modal.Footer>
      ...
      >
        <Modal.Header>
          <Modal.Title>Dokumen Perjalanan</Modal.Title>
