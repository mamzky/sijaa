...
import { Button, Form, Modal, Spinner, CloseButton } from 'react-bootstrap'
...
function Product() {
  ...
  const deleteProduct = async () => {
    ...
    const productDoc = doc(db, PRODUCT_COLLECTION, selectedProduct?.id);
    await deleteDoc(productDoc).then(() => {
      addLog('DELETE PRODUCT', `${localStorage.getItem(Constant.USERNAME)} deleted product "${selectedProduct?.product_name}"`);
  ...
  return (
    <div>
      ...
      <Modal show={showDeleteModal} onHide={() => setShowDeleteModal(false)} size="md" aria-labelledby="contained-modal-title-vcenter" centered>
        ...
        <Modal.Footer>
          <p style={{ width: '100%', textAlign: 'center' }}>Apakah Yakin Ingin Menghapus Produk?</p>
          <Button variant="danger" onClick={() => setShowDeleteModal(false)}>Batal</Button>
          <Button variant="success" onClick={() => deleteProduct()}>Ya, Hapus</Button>
        </Modal.Footer>
      </Modal>
                    ...
                    return (
                      ...
                      >
                        ...
                        <td>
                          <div style={{ display: 'flex', flexDirection: 'row' }}>
                            <Button
                              style={{ width: '100%', marginTop: 10, justifyContent: 'center', zIndex: 20 }}
                              variant="danger"
                              key={'id'}
                              onClick={(e) => {
                                e.stopPropagation()
